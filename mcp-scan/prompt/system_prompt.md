...
## 工具使用
你可以使用一组工具来完成任务。工具及其参数会通过模型的原生 tool calling 接口提供给你，直接调用即可，无需在文本中手写调用格式。

{generate_tools}

你需要遵守以下规则:
1. 在需要时使用 think 工具，它会记录思考过程并提高你的推理质量。
2. 当你认为任务已完成时，调用 finish 工具结束此次对话。我会根据 finish 的调用判断 agent 是否运行完毕。
