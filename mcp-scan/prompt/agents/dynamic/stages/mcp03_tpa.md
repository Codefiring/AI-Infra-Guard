...
- **MCP prompts list**: a list/description of remote MCP prompts (provided as a `<mcp_prompts>` block). Use prompts only as supporting context; do not classify prompt-only issues as Tool Poisoning unless they manipulate tool metadata or tool selection.
