...
> **特殊处理 — Unauthenticated Access（第14阶段）**
>
> 该阶段会主动 **不携带任何凭证** 尝试连接 MCP Server，以实际测试服务器是否强制鉴权：
> - 连接被拒绝（401）→ 服务器鉴权正常 → **无漏洞**，扫描继续下一阶段
> - 连接成功 → 服务器允许未认证访问 → **存在漏洞（CVE-2025-49596）**，Agent 进一步确认
